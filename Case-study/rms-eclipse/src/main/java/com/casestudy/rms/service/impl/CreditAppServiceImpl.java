...
public class CreditAppServiceImpl implements CreditAppService {
	...
	public ResponseModel submitCreditAppForm(CreditPolicyMappingDTO creditApplication, int borrowerId) {
		...
		try {
			...
			LOGGER.error("CreditAppService :: submitCreditAppForm :: Arithmetic Exception");
		}
		if (creditScore >= ApplicationConstant.APP_REJECT_LOWER_LIMIT
				&& creditScore <= ApplicationConstant.APP_REJECT_UPPER_LIMIT) {
			creditAppDAO.updateCreditScore(maxId, creditScore, ApplicationConstant.STATUS_REJECT);
		} else if (creditScore >= ApplicationConstant.APP_HOLD_LOWER_LIMIT
				&& creditScore <= ApplicationConstant.APP_HOLD_UPPER_LIMIT) {
			creditAppDAO.updateCreditScore(maxId, creditScore, ApplicationConstant.STATUS_HOLD);
		} else {
			creditAppDAO.updateCreditScore(maxId, creditScore, ApplicationConstant.STATUS_APPROVED);
		}
	...
	public List<BorrowerAllCreditAppDTO> viewPreviousUpdatedApplication(int faId) {
		...
		for (int i = 0; i < creditAppLst.size(); i++) {
			...
			if (creditAppLst.get(i).getApplicationStatus() == ApplicationConstant.STATUS_APPROVED) {
				creditApp.setApplicationStatus("APPROVED");
			} else {
				creditApp.setApplicationStatus("REJECTED");
			}
